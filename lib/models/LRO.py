...
class LRO(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', downsample=4, mean_size=None, c1=64, c2=64, k=3, s=1):
        ...
        self.dimension_reduction = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=True)
        ...
        self.size_2d = nn.Sequential(nn.Conv2d(channels[self.first_level], self.head_conv, kernel_size=3, padding=1, bias=True),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(self.head_conv, 2, kernel_size=1, stride=1, padding=0, bias=True))
        self.estimated_depth = nn.Sequential(nn.Conv2d(channels[self.first_level], self.head_conv, kernel_size=3, padding=1, bias=True),
                                    nn.ReLU(inplace=True),
        ...
        # self.fill_fc_weights(self.estimated_depth)
    ...
    def forward(self, input, coord_ranges,calibs, targets=None, K=50, mode='train'):
        device_id = input.device    #input  bachsize*3*384*1280
        feat = self.backbone(input)  # feat 6*bachsize*16*384*1280
        feat[self.first_level:]
        feat = self.feat_up(feat[self.first_level:])  #bachsize*64*96*320
        ...
        ret['estimated_depth']=self.estimated_depth(feat)

        # 4*64*96*320
        #CAM
        # CAM(c1=64, c2=64, k=3, s=1).forward(feat)

        x1 = self.dilate_1(feat)    #4*21*96*320
        x2 = self.dilate_3(feat)    #4*21*96*320
        x3 = self.dilate_5(feat)    #4*22*96*320

        # feat = torch.cat((x1, x2, x3), 1))


        #将CAM提取的特征与原特征cat拼接 在通道维度上
        feat = torch.cat((feat, torch.cat((x1, x2, x3), 1)), 1)
        feat = self.dimension_reduction(feat)
    ...
    def get_roi_feat_by_Lmask(self,feat,box2d_maps,inds,mask,calibs,coord_ranges,cls_ids):
        BATCH_SIZE,_,HEIGHT,WIDE = feat.size()
        device_id = feat.device
        num_masked_bin = mask.sum()
        res = {}

        max_out = self.mlp(self.max_pool(feat))
        avg_out = self.mlp(self.avg_pool(feat))
        channel_out = self.sigmoid(max_out + avg_out)
        feat = channel_out * feat

        max_out, _ = torch.max(feat, dim=1, keepdim=True)
        avg_out = torch.mean(feat, dim=1, keepdim=True)
        spatial_out = self.sigmoid(self.conv(torch.cat([max_out, avg_out], dim=1)))
        feat = spatial_out * feat
    ...
    def get_roi_feat_by_Smask(self, feat, box2d_maps, inds, mask, calibs, coord_ranges, cls_ids):
        BATCH_SIZE, _, HEIGHT, WIDE = feat.size()
        device_id = feat.device

        y = self.avg_pool(feat)
        y_out = self.fc1(y)
        feat= feat * y_out
    ...
    def get_roi_feat(self,feat,inds,mask,ret,calibs,coord_ranges,cls_ids):
        ...
        #分类模块
        if ret['estimated_depth']> 60:
            # 深度大于60为远距离目标
           res = self.get_roi_feat_by_Lmask(feat, box2d_maps, inds, mask, calibs, coord_ranges, cls_ids)
        else:
            # 深度小于60为近距离目标
           res = self.get_roi_feat_by_Smask(feat, box2d_maps, inds, mask, calibs, coord_ranges, cls_ids)
